...
@section('content')
    ...
    <div class="content">
        ...
        <div class="grid grid-cols-12 gap-6 mt-5">
            ...
            @endif
            <!-- BEGIN: Form Layout -->
                <form action="{{route('barbell.update', $barbell->id)}}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="intro-y box p-5">
                        <div class="mt-3">
                            <label for="crud-form-sort" class="form-label">{{__('messages.sort')}} </label>
                            <input id="crud-form-sort" name="sort" value="{{$barbell->sort}}" type="number" class="form-control w-full" placeholder="{{__('messages.sort')}} ">
                        </div>

                        <div class="mt-3">
                            <label for="crud-form-kg" class="form-label">Вес </label>
                            <input id="crud-form-kg" name="kg" value="{{$barbell->kg}}" type="text" class="form-control w-full" placeholder="100">
                        </div>

                        <div class="mt-3">
                            <label for="crud-form-active" class="form-label">Показывать </label>
                            <input id="crud-form-active"@if($barbell->active)checked @endif name="active" type="checkbox" class="form-control w-full" value="1" />
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <select name="type" id="crud-form-type">
                                ...
                                <option value="2"@if($barbell->type == 2) selected @endif>Гриф</option>
                            </select>
                        </div>
