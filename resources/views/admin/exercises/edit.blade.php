            ...
            <div class="intro-y col-span-12 lg:col-span-6">
                ...
                <form action="{{ route('exercise.update', $exercise->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="intro-y box p-5">
                        ...
                        <div class="mt-3">
                            <label for="crud-form-trigger" class="form-label">{{ __('messages.trigger') }} </label>
                            <input id="crud-form-trigger" name="trigger" type="text" class="form-control w-full"
                                placeholder="{{ __('messages.trigger') }} " value="{{ $exercise->trigger }}" />
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <textarea id="crud-form-description" name="description" type="text" class="form-control w-full"
                                placeholder="{{ __('messages.description') }} ">{{ $exercise->description }}</textarea>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <select name="apparatus_id" id="apparatus_id" class="form-control w-full">
                                <option value="">-- select --</option>
                                @forelse (App\Models\Apparatus::get() as $apparat)
                                    <option value="{{ $apparat->id }}"
                                        {{ $apparat->id == $exercise?->apparatus?->id ? 'selected' : '' }}>
                                        {{ $apparat->name }}</option>
                                @empty
                                @endforelse
                            </select>
